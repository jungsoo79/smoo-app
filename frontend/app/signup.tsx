<<<<<<< Updated upstream
export { default } from '@/features/auth/SignupScreen';
=======
...
export default function SignupScreen() {
  ...
  const canSubmit = isEmailValid && isPasswordStrong && isPasswordMatched && isNicknameValid;
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      if (message === 'OTP_EXPIRED') {
        setSignupError('인증번호가 만료되었습니다. 회원가입을 다시 요청해주세요.');
      } else if (message === 'OTP_INVALID') {
        setSignupError('인증번호가 올바르지 않습니다.');
      } else {
        setSignupError('이메일 인증에 실패했습니다.');
      }
    } finally {
        ...
        <ScrollView
          ...
          <View style={styles.form}>
            <View style={styles.fieldGroup}>
              ...
              {isCodeSent ? (
                <Text style={styles.helperText}>이메일로 받은 인증번호를 입력해 확인하세요.</Text>
              ) : null}
            </View>
            ...
            <View style={styles.fieldGroup}>
              ...
              <View
                ...
                />
                <Pressable
                  disabled={!isCodeSent || isCodeVerified || isVerifyingCode}
                  onPress={handleVerifyCode}
                  style={styles.codeButton}>
                  <Text
                    style={[
                      styles.codeButtonText,
                      (!isCodeSent || isCodeVerified || isVerifyingCode) && styles.disabledText,
                    ]}>
                    {isVerifyingCode ? '...' : '확인'}
                  </Text>
                </Pressable>
            ...
            <Pressable
              ...
              <Text style={[styles.primaryButtonText, (!canSubmit || isSigningUp) && styles.primaryButtonTextDisabled]}>
                {isSigningUp ? '요청 중...' : isCodeSent ? '인증번호 다시 받기' : '회원가입'}
              </Text>
            </Pressable>
          </View>
